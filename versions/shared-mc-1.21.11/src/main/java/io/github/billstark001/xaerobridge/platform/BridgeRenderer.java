...
/** Minecraft 1.21.11 render adapter. */
public final class BridgeRenderer {
    ...
    public static boolean wasExactRendered() {
        return exactRendered;
    }

    public static void renderExact(Screen screen, GuiGraphics graphics, int width, int height) {
        exactRendered = true;
    ...
    public static void renderTail(Screen screen, GuiGraphics graphics, int width, int height) {
        if (!exactRendered && BridgeSettings.isTailFallbackEnabled()) {
            renderMap(screen, graphics, width, height);
        }
    ...
    private static void renderMap(Screen screen, GuiGraphics graphics, int width, int height) {
        ...
        OverlayRegistry.renderMap(new MapOverlayContext(
                graphics::fill, width, height, cameraX, cameraZ, scale / guiScale, dimension));
    }
